...
if [ "$upload" = true ]; then
    scp ${LOCAL_PATH}/opencv_test_core ${LOCAL_PATH}/opencv_perf_core ${USER}@${REMOTE_IP}:${REMOTE_PATH}
fi

if [ "$run_test" = true ]; then
    ssh ${USER}@${REMOTE_IP} "${REMOTE_PATH}/opencv_test_core --gtest_filter=\"*MeanStdDev*\" " 
fi

if [ "$run_perf" = true ]; then
    ssh ${USER}@${REMOTE_IP} "${REMOTE_PATH}/opencv_perf_core --gtest_filter=\"*meanStdDev*\" --gtest_output=xml:${REMOTE_PATH}/result.xml"
